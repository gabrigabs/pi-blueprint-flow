...
export function registerActionRoutes(app: FastifyInstance): void {
  app.post<{ Params: { id: string }; Body: { summary?: string } }>(
    ...
    async (req, reply) => {
      ...
      if (currentIdx === -1) {
        return reply.code(400).send({ error: "invalid_state", message: "Invalid current step" });
      }
      ...
      if (currentIdx === FLOW_STEPS.length - 1) {
        ...
        bus.emit("feature:updated", { id, step: feature.current_step, status: "done" });
      ...
      bus.emit("step:advanced", { featureId: id, from: feature.current_step, to: nextStep });

      const updated = db.prepare("SELECT * FROM features WHERE id = ?").get(id);
      const steps = db.prepare("SELECT * FROM steps WHERE feature_id = ? ORDER BY rowid").all(id);
  ...
  app.post<{ Params: { id: string } }>(
    ...
    async (req, reply) => {
      ...
      const currentIdx = FLOW_STEPS.indexOf(feature.current_step as FlowStep);
      if (currentIdx <= 0) {
        return reply.code(400).send({ error: "invalid_state", message: "Already at first step" });
      }
      ...
      bus.emit("step:back", { featureId: id, from: feature.current_step, to: prevStep });
  ...
  app.patch<{ Params: { id: string }; Body: { status: StepStatus } }>(
    ...
    async (req, reply) => {
      ...
      const validStatuses: StepStatus[] = ["pending", "running", "needs_user", "blocked", "done", "rejected"];
      ...
      if (!step) {
        return reply.code(404).send({ error: "not_found", message: "Step not found" });
      }
  ...
  app.post<{ Params: { id: string }; Body: { agentRunSettings?: Partial<AgentRunSettings> } }>(
    "/api/features/:id/run-step",
    async (req, reply) => {
      const { id } = req.params;
      ...
      return reply.send({
        ...
        settings,
        settingsId,
        message: `Step "${STEP_LABELS[feature.current_step as FlowStep]}" queued with effort: ${settings.effortLevel}`,
      });
    }
  );
}
