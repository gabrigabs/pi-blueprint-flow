...
export function registerActionRunRoutes(app: FastifyInstance): void {
	...
	app.get<{ Params: { id: string } }>("/api/action-runs/:id", async (req, reply) => {
		const run = getActionRun(req.params.id);
		if (!run) {
			return reply.code(404).send({ error: "not_found", message: "Action run not found" });
		}
	...
	app.post<{ Body: RunBlueprintActionInput }>("/api/action-runs", async (req, reply) => {
		const input = req.body;

		if (!input.actionType) {
			return reply.code(400).send({ error: "validation", message: "actionType is required" });
		}
	...
	app.post<{ Params: { id: string } }>("/api/action-runs/:id/cancel", async (req, reply) => {
		const bridge = getPiBridge();
		...
		if (!cancelled) {
			...
			return reply.code(409).send({ error: "cannot_cancel", message: "Action run is not in a cancellable state" });
		}
	...
	app.get("/api/bridge/status", async (_req, reply) => {
		const bridge = getPiBridge();
		return reply.send({ status: bridge.getStatus() });
	});
}
