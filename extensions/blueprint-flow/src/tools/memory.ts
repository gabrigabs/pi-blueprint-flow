...
export const saveMemoryTool = {
  ...
  parameters: Type.Object({
    ...
    category: Type.String({
      description: "Memory category: decision, pattern, constraint, learning, convention, architecture, domain",
    }),
    content: Type.String({ description: "The knowledge to remember (be specific and actionable)" }),
  ...
  execute: async (
    _toolCallId: string,
    params: { project_id: string; category: string; content: string; source_feature_id?: string }
  ) => {
    const db = getDb();
    const id = nanoid(12);
    ...
    ).run(id, params.project_id, params.category, params.content, params.source_feature_id ?? null);

    // Update FTS index
    db.prepare("INSERT INTO memories_fts (rowid, content, category) VALUES (last_insert_rowid(), ?, ?)").run(
      params.content,
      params.category
    );

    bus.emit("memory:saved", { id, projectId: params.project_id, category: params.category });
...
export const searchMemoryTool = {
  ...
  execute: async (
    _toolCallId: string,
    params: { project_id: string; query: string; category?: string; limit?: number }
  ) => {
    const db = getDb();
