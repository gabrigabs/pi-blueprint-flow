...
export const advanceStepTool = {
  ...
  parameters: Type.Object({
    ...
    summary: Type.Optional(
      Type.String({ description: "Brief summary of what was accomplished in this step" })
    ),
  }),
  execute: async (_toolCallId: string, params: { feature_id: string; summary?: string }) => {
    const db = getDb();
    ...
    if (currentIdx === -1) {
      return {
        content: [{ type: "text" as const, text: `Invalid current step: ${feature.current_step}` }],
    ...
    return {
      ...
      details: { featureId: params.feature_id, previousStep: feature.current_step, currentStep: nextStep },
    };
  },
};
...
export const resetStepTool = {
  ...
  parameters: Type.Object({
    feature_id: Type.String({ description: "Feature ID" }),
    target_step: Type.String({ description: "Step name to reset to (e.g. 'research', 'spec')" }),
  }),
  execute: async (_toolCallId: string, params: { feature_id: string; target_step: string }) => {
    const db = getDb();
