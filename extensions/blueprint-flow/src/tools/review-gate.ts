...
function evaluateDiscipline(artifacts: Artifact[], feature: Feature): DisciplineScores {
...
function evaluateSurgicalChanges(content: string, artifacts: Artifact[]): number {
  let score = 0.7;

  const planArtifacts = artifacts.filter((a) => a.type === "implementation_plan");
  if (planArtifacts.length > 0) {
    score += 0.1;

    const planContent = planArtifacts.map((a) => a.content).join("\n").toLowerCase();
    if (planContent.includes("files to modify") || planContent.includes("files to create")) {
      score += 0.1;
    }
  }
...
function evaluateVerification(content: string): number {
  ...
  const riskIndicators = ["risk", "limitation", "known issue", "caveat", "trade-off", "tradeoff"];
...
function formatDisciplineGate(scores: DisciplineScores): string {
  ...
  lines.push(`  **Overall discipline: ${scoreBar(scores.overallDisciplineScore)}**`);
...
export const reviewGateTool = {
  ...
  execute: async (
    ...
    const status = issues.length === 0 && disciplineOk ? "PASS" : score >= 70 && disciplineOk ? "WARN" : "FAIL";
