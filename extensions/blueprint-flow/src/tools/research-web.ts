...
export const researchWebTool = {
	...
	parameters: Type.Object({
		...
		query: Type.Optional(
			Type.String({ description: "Search query (for search mode)" }),
		),
		url: Type.Optional(
			Type.String({ description: "URL to fetch (for fetch mode)" }),
		),
	...
	execute: async (
		...
		if (params.mode === "fetch") {
			if (!params.url) {
				return {
					content: [
						{ type: "text" as const, text: "URL required for fetch mode" },
					],
				};
			}
			...
			if (!validateUrl(params.url)) {
				return {
					content: [
						{
							type: "text" as const,
							text: "URL rejected: only public http/https URLs are allowed",
						},
					],
				};
			}
			...
			try {
				...
				return {
					content: [
						{ type: "text" as const, text: `Fetch failed: ${err.message}` },
					],
		...
		if (params.mode === "search") {
			if (!params.query) {
				return {
					content: [
						{ type: "text" as const, text: "Query required for search mode" },
					],
				};
			}
		...
		return {
			content: [
				{
					type: "text" as const,
					text: `Invalid mode: "${params.mode}". Use 'search' or 'fetch'.`,
				},
			],
		};
	},
