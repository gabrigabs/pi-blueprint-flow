...
function scanDirectory(dirPath: string, repoPath: string, type: string): AgenticFile[] {
...
function extractRules(content: string): string[] {
  ...
  for (const line of lines) {
    ...
    const isRule =
      ...
      trimmed.startsWith("* ") ||
      trimmed.startsWith("• ") ||
      /^(always|never|prefer|avoid|use|do not|don't|ensure|must|should)/i.test(trimmed);
