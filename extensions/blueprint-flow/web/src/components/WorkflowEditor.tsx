...
import { X, Plus, GripVertical, Trash2, Save, RotateCcw } from "lucide-react";

export function WorkflowEditor() {
  const { selectedProjectId, activeWorkflow, setActiveWorkflow, setWorkflows, closeModal } = useStore();
  ...
  const handleStepChange = (index: number, field: keyof WorkflowStep, value: string | boolean) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (activeWorkflow && !activeWorkflow.is_default) {
        // Update existing
        await api.workflows.update(activeWorkflow.id, { name, description, steps });
      } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-2xl rounded-xl border border-white/[0.06] bg-zinc-900 shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-white/[0.06] px-5 py-4">
          <div>
            <h2 className="text-base font-semibold text-zinc-100">Workflow Editor</h2>
            <p className="text-xs text-zinc-500 mt-0.5">Configure the step sequence for this project</p>
          </div>
          <button
            type="button"
            onClick={closeModal}
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {steps.map((step, i) => (
                <div
                  key={i}
                  className="flex items-center gap-2 rounded-lg border border-white/[0.04] bg-zinc-800/30 px-2 py-1.5"
                >
                  ...
                  <input
                    type="text"
                    value={step.name}
                    onChange={(e) => handleStepChange(i, "name", e.target.value.toLowerCase().replace(/\s+/g, "_"))}
