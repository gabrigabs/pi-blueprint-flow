  ...
  async function refreshData() {
    ...
    try {
      ...
      if (store.selectedProjectId) {
        const featuresRes = await fetch(`/api/projects/${store.selectedProjectId}/features`);
        if (featuresRes.ok) {
      ...
      if (store.selectedFeatureId) {
        ...
        if (artifactsRes.ok) useStore.getState().setArtifacts(await artifactsRes.json());
        if (interviewsRes.ok) useStore.getState().setInterviews(await interviewsRes.json());
      }
    } catch {
